        ...
        try {
          ...
          if (tableIdRaw !== undefined && tableIdRaw !== null) {
            ...
            if (!isNaN(tableId)) {
              const table = await TableAsset.findByPk(tableId).catch((e) => {
                console.warn(
                  `Failed to fetch table ${tableId} for res ${reservation.id}:`,
                  e.message,
                );
                return null;
              });
              if (table) {
                ...
                if (table.gameid) {
                  const game = await Game.findByPk(table.gameid).catch(
                    (e) => null,
                  );
                  if (game) {
          ...
          if (reservation.userId || reservation.userid) {
            ...
            const user = await User.findByPk(userId).catch((e) => {
              console.warn(
                `Failed to fetch user ${userId} for res ${reservation.id}:`,
                e.message,
              );
              return null;
            });
          ...
          console.error(
            `Error enriching reservation ${reservation.id}:`,
            innerErr.message,
          );
        }
    ...
    try {
      ...
      if (activeSession) {
        ...
        const activeStart = new Date(
          activeSession.starttime || activeSession.start_time,
        );
        const activeDuration =
          activeSession.durationminutes || activeSession.duration_minutes;
      ...
      if (conflictingReservation) {
        ...
        const holderName =
          ...
          conflictingReservation.customer_name ||
          "Another Customer";
...
router.put("/:id", auth, stationContext, async (req, res) => {
  try {
    ...
    if (updates.customer_name)
      allowedUpdates.customerName = updates.customer_name;
    if (updates.customer_phone)
      allowedUpdates.customerPhone = updates.customer_phone;

    // Allow editing booking details
    if (updates.table_id) allowedUpdates.tableId = updates.table_id;
    if (updates.booking_type)
      allowedUpdates.booking_type = updates.booking_type;
    ...
    if (updates.reservation_date || updates.start_time) {
      ...
      if (r.fromTime) {
        allowedUpdates.toTime = new Date(
          new Date(r.fromTime).getTime() + updates.duration_minutes * 60000,
        );
      }
    }
